...
export const Route = createFileRoute('/auth/email-change-verify')({
...
function EmailChangeVerifyComponent() {
  ...
  const [message, setMessage] = useState('正在验证新邮箱...')
  ...
  useEffect(() => {
    ...
    const verify = async () => {
      try {
        const res: any = await fetchClient('/api/auth/email-change-verify', {
          method: 'POST',
          body: { token },
        })
        setStatus('success')
